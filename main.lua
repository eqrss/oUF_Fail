...
local defaultSettings = {
 ...
 --   enableHideMacroText = false,
    ...
    enableChatMouseoverItemTooltip = true,
...
function settingsInterface:Initialize()
    ...
    self.bigTitle:SetText(self.name .. " v" .. C_AddOns.GetAddOnMetadata("oUF_Fail", "Version"))
    ...
    local category = Settings.RegisterCanvasLayoutCategory(self, "oUF_Fail")
    category.ID = "oUF_Fail"
...
settingsInterface:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == "oUF_Fail" then
        -- Init
        InitVariables()
