...
if( not HealComm.spellToID ) then
	HealComm.spellToID = setmetatable({}, {
		__index = function(tbl, index)
			...
			local offset, numSpells = select(3, C_SpellBook.GetSpellBookSkillLineInfo(GetNumSpellTabs()))
...
-- C_C_UnitAuras.GetAuraDataByIndexs.GetBuffDataByIndex priortizes our buffs over everyone elses when there is a name conflict, so yay for that
local function unitHasAura(unit, name)
	return select(8, C_C_UnitAuras.GetAuraDataByIndexs.GetBuffDataByIndex(unit, name)) == "player"
end
...
if( playerClass == "DRUID" ) then
	LoadClassData = function()
		-- Rejuvenation
		local Rejuvenation = C_Spell.GetSpellInfo(774)
		...
		-- Regrowth
		local Regrowth = C_Spell.GetSpellInfo(8936)
		...
		-- Lifebloom
		local Lifebloom = C_Spell.GetSpellInfo(33763)
		...
		-- Wild Growth
		local WildGrowth = C_Spell.GetSpellInfo(48438)
		...
		-- Healing Touch
		local HealingTouch = C_Spell.GetSpellInfo(5185)
		...
		-- Nourish
		local Nourish = C_Spell.GetSpellInfo(50464)
		spellData[Nourish] = {coeff = 0.358005, levels = {80}, averages = {avg(1883, 2187)}}
		-- Tranquility
		local Tranquility = C_Spell.GetSpellInfo(740)
		...
		CalculateHotHealing = function(guid, spellID)
			local spellName, spellRank = C_Spell.GetSpellInfo(spellID)
...
if( playerClass == "PALADIN" ) then
	LoadClassData = function()
		...
		--local FlashofLight = C_Spell.GetSpellInfo(19750)
		--hotData[FlashofLight] = true
		
		-- Spell data
		-- Holy Light
		local HolyLight = C_Spell.GetSpellInfo(635)
		...
		-- Healing Light (Add)
		local HealingLight = C_Spell.GetSpellInfo(20237)
		talentData[HealingLight] = {mod = 0.04, current = 0}
		-- Divinity (Add)
		local Divinity = C_Spell.GetSpellInfo(63646)
		talentData[Divinity] = {mod = 0.01, current = 0}
		-- Touched by the Light (Add?)
		local TouchedbytheLight = C_Spell.GetSpellInfo(53592)
		...
		local BeaconofLight = C_Spell.GetSpellInfo(53563)
		-- 100% chance to crit
		local DivineFavor = C_Spell.GetSpellInfo(20216)
		-- Seal of Light + Glyph = 5% healing
		local SealofLight = C_Spell.GetSpellInfo(20165)
		-- Divine Illumination, used in T10 holy
		local DivineIllumination = C_Spell.GetSpellInfo(31842)
...
if( playerClass == "PRIEST" ) then
	LoadClassData = function()
		-- Hot data
		local Renew = C_Spell.GetSpellInfo(139)
		...
		--local GlyphofPoH = C_Spell.GetSpellInfo(56161)
		--hotData[GlyphofPoH] = {isMulti = true, interval = 3}
		
		-- Spell data
		-- Greater Heal
		local GreaterHeal = C_Spell.GetSpellInfo(2060)
		...
		-- Prayer of Healing
		local PrayerofHealing = C_Spell.GetSpellInfo(596)
		...
		-- Flash Heal
		local FlashHeal = C_Spell.GetSpellInfo(2061)
		...
		AuraHandler = function(unit, guid)
			local stack, _, _, _, caster = select(4, C_C_UnitAuras.GetAuraDataByIndexs.GetBuffDataByIndex(unit, Grace))
...
if( playerClass == "SHAMAN" ) then
	LoadClassData = function()
		...
		-- Healing Wave
		local HealingWave = C_Spell.GetSpellInfo(331)
		...
		-- Lesser Healing Wave
		local LesserHealingWave = C_Spell.GetSpellInfo(8004)
		...
		AuraHandler = function(unit, guid)
			...
			if( C_C_UnitAuras.GetAuraDataByIndexs.GetBuffDataByIndex(unit, EarthShield) ) then
...
local function updateDistributionChannel()
    ...
    -- Update event registrations based on the new distribution
...
function HealComm:UNIT_AURA(unit)
	...
	local id = 1
	while( true ) do
		local name, rank, icon, stack, _, _, _, _, _, _, spellID = C_UnitAuras.GetAuraDataByIndex(unit, id, "HELPFUL")
	...
	id = 1
	while( true ) do
		local name, rank, icon, stack, _, _, _, _, _, _, spellID = C_UnitAuras.GetAuraDataByIndex(unit, id, "HARMFUL")
		if( not name ) then break end
...
-- Save the currently equipped relic/weapon
local function GetRelicSlotForSpec()
    local specIndex = GetSpecialization()
    if not specIndex then return nil end
    
    -- Get the appropriate inventory slot ID based on spec
    local slot = 16 -- Default to main hand slot
    if specIndex == 3 then -- Check if it's the third spec
        slot = 17 -- Use off-hand slot for certain specs
    end
...
function HealComm:PLAYER_EQUIPMENT_CHANGED()
    ...
    -- Check relic/weapon
    local relicSlot = GetRelicSlotForSpec()
    if relicSlot then
        local relic = GetInventoryItemLink("player", relicSlot)
...
local function findAura(casterGUID, spellName, spellRank, inc, ...)
	for i=1, select("#", ...), inc do
		...
		if( unit and UnitIsVisible(unit) ) then
			local id = 1
			while( true ) do
				local name, rank, _, stack, _, duration, endTime, caster = C_C_UnitAuras.GetAuraDataByIndexs.GetBuffDataByIndex(unit, id)
				if( not name ) then break end
...
function HealComm:PLAYER_LOGIN()
    ...
    -- Update distribution channel and clear any pending data
    updateDistributionChannel()
    clearGUIDData()
