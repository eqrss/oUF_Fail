...
-- Font ayarlarını uygulama fonksiyonu
local function ApplyFontSettings(fontPath, size)
    for i = 1, NUM_CHAT_WINDOWS do
        local cf = _G["ChatFrame"..i]
        if cf then
            cf:SetFont(fontPath, size, "")
        end
    end

    -- Temporary chat frames (whispers etc.)
    for i = 11, 25 do
        local cf = _G["ChatFrame"..i]
        if cf then
            cf:SetFont(fontPath, size, "")
    ...
    -- Varsayılan sohbet penceresi boyutunu ayarla
    FCF_SetChatWindowFontSize(nil, ChatFrame1, size)
end

-- Font ayarlarını yükleme
local function LoadFontSettings()
    local fontPath = "Fonts\\texgyreadventor-bold.TTF" -- Varsayılan font
    local size = 11 -- Varsayılan boyut

    -- Font ayarlarını uygula
    ApplyFontSettings(fontPath, size)
end

-- Event frame for loading settings
local eventFrame = CreateFrame("Frame")
eventFrame:RegisterEvent("ADDON_LOADED")

eventFrame:SetScript("OnEvent", function(self, event, arg1)
    if event == "ADDON_LOADED" and arg1 == addonName then
        C_Timer.After(0.5, LoadFontSettings)
    end
end)
